...
int config_topic_info(neu_plugin_t *plugin)
{
    ...
    char          *prefix =
        (char *) malloc((1 + 2 + strlen(device_info->product_id) + strlen(device_info->device_id)) * sizeof(char));
    sprintf(prefix, "/%s/%s", device_info->product_id, device_info->device_id);
    char **s_topics      = (char **) malloc(sizeof(char *) * topic_info->s_topic_count);
    ...
    char **p_topics      = (char **) malloc(sizeof(char *) * topic_info->p_topic_count);
    ...
    for (int i = 0; i < topic_info->s_topic_count; i++) {
        plog_notice(plugin, "config s_topic[%d] : %s", i, topic_info->s_topics[i]);
    }
    for (int j = 0; j < topic_info->p_topic_count; j++) {
        plog_notice(plugin, "config p_topic[%d] : %s", j, topic_info->p_topics[j]);
    }
...
void init_mqtt_quic_client(neu_plugin_t *plugin)
{
    ...
    plugin->client->device_info = (device_info_t *) malloc(sizeof(device_info_t));
...
void add_connection_status_checker(neu_plugin_t *plugin)
{
    neu_event_timer_param_t param = { .second      = 3,
                                      .millisecond = 0,
                                      .cb          = check_connect_status_callback,
