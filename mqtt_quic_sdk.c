...
static nng_msg *compose_publish(int qos, char *topic, char *payload)
{
    ...
    property *p4 = mqtt_property_set_value_str(RESPONSE_TOPIC, "aaaaaa",
                                               strlen("aaaaaa"), true);
    ...
    property *p6 = mqtt_property_set_value_strpair(USER_PROPERTY, "aaaaaa",
                                                   strlen("aaaaaa"), "aaaaaa",
                                                   strlen("aaaaaa"), true);
    mqtt_property_append(plist, p6);
    property *p7 = mqtt_property_set_value_str(CONTENT_TYPE, "aaaaaa",
                                               strlen("aaaaaa"), true);
...
int client_init(neu_plugin_t *plugin){
    ...
    plog_debug(plugin,"开启 MQTT 客户端连接");
    if ((rv = nng_mqttv5_quic_client_open_conf(&client->sock, client->url,
                                               client->conf)) != 0) {
        rv = -1;
...
int client_unsubscribe(neu_plugin_t *plugin) {
    ...
    nng_msg *msg = compose_unsubscribe_multiple(topic_info->s_topics,
                                                topic_info->s_topic_count);
